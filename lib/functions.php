...
*/
	...
	if (($user) && ($user instanceof ElggUser)) {
		...
		// Send validation email
		$subject = elgg_echo('email:validate:subject', array($user->name, $site->name));
		$body = elgg_echo('email:validate:body', array($admin->name, $user->name, $user->username, $user->email, $ip_address, $geostring, $ip_address, $user->email, $link, $site->url . "/admin/users/unvalidated", $site->name, $site->url));
